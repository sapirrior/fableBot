# Fable Bot — Text Styling Standards (The Swarm Terminal)
...
### 1.1 Prefix / Status Emoji (`[EMOJI]`)
All main responses must begin with a theme-appropriate emoji wrapped in square brackets:
...
### 1.2 Separator (`::`)
The status block and username are separated from the main text body by a double colon `::` with single spaces on either side.
...
> Experience generated: **+15 XP**
```

### 2.5 Sell Command
```
[💵] **sapirrior** :: Successfully sold **5x** `ant` 🐜!
> Fables generated: **+50 Fables**
```

### 2.6 Release Command
```
[⌬] **sapirrior** :: Released **1x** `scorpion` 🦂 back into the wild.
```

### 2.7 Profile Command (Embed Description)
Embeds should follow a clean terminal metadata block style:
```
