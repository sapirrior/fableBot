 ...
 * @returns {Promise<boolean>} true if command should proceed, false if blocked
 */
export async function check(message, commandObj) {
  ...
  if (cSpam.count >= 6) {
    ...
    if (cSpam.count === 6) {
      sender.reply(message, timerEmoji, ', this channel is sending commands too fast! Please slow down.').catch(() => {});
    }
    return false;
  }
  ...
  if (uSpam.count >= 3) {
    ...
    if (uSpam.count === 3) {
      sender.reply(message, timerEmoji, ", slow down! You are typing commands too fast.").catch(() => {});
    }
    return false;
  }
  ...
  try {
    ...
    if (userBan) {
      sender.error(message, ", you're banned from this command! >:c").catch(() => {});
    ...
    if (!commandObj.adminBypass) {
      const disabledCommand = query('checkDisabledCommand').get(channelId, command);
      if (disabledCommand) {
        sender.error(message, ', that command is disabled on this channel!').catch(() => {});
        uSpam.blockedUntil = now + 5000;
        return false;
      }
    }
    ...
    // Fail open on database errors to avoid completely bricking bot functionality
  }
