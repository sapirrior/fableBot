  ...
  async execute(client, message, args, ctx) {
    ...
    if (collectionRows.length === 0) {
      const resp = target.id === message.author.id
        ? "🪹 Your collection is empty! Start catching using `f!catch`."
    ...
    for (const row of collectionRows) {
      ...
      if (spec) {
        description += `${spec.emoji} **${spec.name}** (x${row.count}) — \`[${spec.rarity.toUpperCase()}]\`\n`;
      }
    }

    const embed = {
      color: parseInt(ctx.config.embedColor.replace('#', ''), 16),
      title: `🪲 ${target.username}'s Insect Collection`,
      description,
      timestamp: new Date()
    };
